...
#We can remove or comment out this section if we are running this on an EC2 with IAM Instance profile attached
if [ "$AWS_PROFILE" = "" ]; then
  echo "No AWS_PROFILE set"
  exit 1
fi

#Region we want to delete the Default VPC from
region=ap-northeast-2

for region in $(aws ec2 describe-regions --region $region | jq -r .Regions[].RegionName); do
